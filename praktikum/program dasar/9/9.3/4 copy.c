...
void input(data[]);
void grade(data[],maks*);
void line(void);
void tampil(data[],maks*);
...
void grade(data grade[],maks *no1)
{
    float grademax=-1;
    for (i = 0; i < amnt; i++)
    {
        grade[i].akhir=grade[i].tugas*0.2 +grade[i].uts*0.4+grade[i].uas*0.4;
        if (grade[i].akhir>grademax)
        {
            no1->akhir = grade[i].akhir;
            grademax=grade[i].akhir;
            strcpy(no1->nama,grade[i].nama);
        }
        if (grade[i].akhir>=80)
        {
            grade[i].grade='A';
...
void tampil(data output[],maks *top)
{
